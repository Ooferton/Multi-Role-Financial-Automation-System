...
class RLStrategy(TradingStrategy):
    """
    Self-modifying RL trading strategy.
    
    Uses:
    - RL Agent for numeric action prediction
    ...
    def __init__(self, name: str, config: Dict, broker: BrokerInterface, model_path: str):
        ...
        self.logger.info(f"Strategy initialized with genome:\n{self.evolver.get_status()}")
    ...
    def on_tick(self, tick: MarketTick) -> Optional[Dict]:
        ...
        # 5. Check Stop Loss / Take Profit (self-modifying thresholds)
        if tick.symbol in self.entry_prices and current_qty > 0:
            ...
            if pnl_pct <= -genome.stop_loss_pct:
                reasoning = self.reasoner.explain_trade(
                    -1.0, 
                    {'rsi_14': current_row['rsi_14'], 'macd': current_row['macd'],
                     'macd_signal': current_row['macd_signal'], 'bb_width': current_row['bb_width'],
        ...
        if abs(current_drawdown) > genome.max_drawdown_pct:
            self.logger.warning(f"⚠️ CIRCUIT BREAKER: Drawdown {current_drawdown:.1%} exceeds {genome.max_drawdown_pct:.1%}. Pausing new trades.")
            return None
                
        # 7. Construct Observation Vector
        indicators = {
        ...
        # 10. Apply genome thresholds and sizing
    ...
    def self_adapt(self):
        """
        Called periodically (e.g. end of day) to let the AI evolve its own strategy.
        """
