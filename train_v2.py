...
def train_v2():
    ...
    symbols = ["SPY", "QQQ", "DIA", "IWM", "TLT", "GLD", "USO", "AAPL", "MSFT", "GOOGL", "AMZN", "FB", "TSLA", "NVDA", "INTC", "AMD", "CSCO", "ORCL", "IBM", "HPQ", "DELL"]
    # Remove duplicates but preserve order if possible
    symbols = list(dict.fromkeys(symbols))
    
    logger.info(f"Fetching 60 days of 5m data for {len(symbols)} symbols...")
    ...
    all_dfs = []
    for s in symbols:
        logger.info(f"  Fetching {s}...")
        try:
            df_s = connector.fetch_historical_df(s, start, end)
            if not df_s.empty:
                all_dfs.append(df_s)
    ...
    logger.info(f"Loaded total {len(df)} data points from {len(all_dfs)} symbols.")
    ...
    existing_model_path = "ml/models/ppo_trading_real_v2.zip"
    if os.path.exists(existing_model_path):
        logger.info(f"🔄 Continuing training from existing model with LR=0.090: {existing_model_path}")
        agent = RLAgentV2(model_path=existing_model_path, training_data=df, learning_rate=0.090)
    else:
        logger.info("🆕 No existing v2 model found. Starting fresh training...")
        # Higher LR for faster convergence with GPU
        agent = RLAgentV2(training_data=df, learning_rate=0.900)
    
    logger.info(f"Observation space: {agent.env.observation_space.shape}")
    logger.info(f"Action space: {agent.env.action_space.shape}")
    
    # 3. Train
    training_duration_minutes = 30  # Longer for richer model
    training_duration_seconds = training_duration_minutes * 60
    
    logger.info(f"Starting Training for {training_duration_minutes} minutes with LR=0.090...")
